...
/*
* Had to work on this on March 7,2017. Couold not reproduce old plots without a lot of tweeking
* Because the errors on points seemed to change - do not know why.
*/
...
void FeedDownFitter::background( string name, int plcIndex, int bin, ofstream &out ){
	...
	if ( "Pi_n" == name ){
		...
		fracFun->SetParLimits( 2, 0.1, 1 );
		fracFun->SetParLimits( 3, 0, 50 );

		fracFun->SetParameters( .1, 0.005, .01, 0.002, 0.9, 20 );
		min=0.2;
		max=1.5;
		// g->Fit( fracFun, "RNQW", "", min, max );
	} else  if ( "Pi_p" == name ){
		INFO( name )
		fracFun->SetParameters( 0.025, 0.35, 0.05, 2.5 );

		fracFun->SetParLimits( 0, 0.0, 1 );
		fracFun->SetParLimits( 1, 0, 5 );
		fracFun->SetParLimits( 2, 0.0, 1 );
		fracFun->SetParLimits( 3, 0, 50 );
		...
		fracFun->SetParLimits( 3, 10, 50 );

		fracFun->SetParameters( .3, 0.5, .1, 20 );
		min=0.2;
		max=1.75;
	} else if ( "P_n" == name ){
		
		fracFun->SetParLimits( 0, 2, 5 );
		fracFun->SetParLimits( 1, 0, 20 );
		fracFun->SetParLimits( 2, 0, 2 );
	
		fracFun->SetParameters( 2, 10.0, 1.3 );
		min=0.1;
		max=1.25;
		
	}

	g->Fit( fracFun, "RNQ", "", min, max );
	g->Fit( fracFun, "RNQ", "", min, max );
	TFitResultPtr fitPointer = g->Fit( fracFun, "QRSM", "", min, max );
