...
void UnitTest( string df, string plc = "Pi" ){
	...
	string test = UT_R_NAMED( pp.project2D( name ), plc + "_p_0_15_2D" );
	INFO( tag, test);
	c1->SetWindowPosition( 0, 0 );

	TCanvas * c2 = newCan( "1D projection of zb" );
	test = UT_R_NAMED( pp.project1D( name, "zb" ), plc + "_p_0_15_zb_1D" );
	INFO( tag, test);
	...
	test = UT_R_NAMED( pp.projectEnhanced( name, "zd", "K", 0.0-0.012, 0.0+0.012 ), plc + "_p_0_15_zd_1D_K" );
	INFO( tag, test);
